...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onSaveInstanceState(@NonNull Bundle outState) {
        ...
        byte[] boardSingDimensional = new byte [9];

        for(int i=0 ; i<3; i++){
            for(int j=0 ; j<3; j++){
                boardSingDimensional[3*i+j]= board [i][j];
            }
        }

        outState.putByteArray("board", boardSingDimensional);
    ...
    protected void onRestoreInstanceState(@NonNull Bundle savedInstanceState) {
        ...
        byte[] boardSingDimensional = savedInstanceState.getByteArray("board");
        for(int i=0; i<9; i++){
            board[i/3][i%3]= boardSingDimensional[i];
        }
